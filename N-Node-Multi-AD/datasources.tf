...
#data "oci_core_vnic_attachments" "all_broker_nodes_vnics" {
#  compartment_id      = "${var.compartment_ocid}"
#  availability_domain = "${lookup(data.oci_identity_availability_domains.ADs.availability_domains[var.AD - 1],"name")}"
#  instance_id = "${oci_core_instance.BrokerNode.*.id}"
#}

#data "oci_core_vnic" "all_broker_nodes_first_vnic" {
#  vnic_id = "${lookup(data.oci_core_vnic_attachments.all_broker_nodes_vnics.*.vnic_attachments[0],"vnic_id")}"
#}
